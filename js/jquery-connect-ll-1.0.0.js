	...
	var methods = {
		init : function( options ) {
			...
			function buildLeftList(e, optionList) {
				...
				str += '</div>' ;
				optionList.each(function(index) {
					e.elements[optionList[index].value] = false;
				});
				return str;
			}
			...
			function buildRightList(e, optionList) {
				...
				str += '</div>' ;
				optionList.each(function(index) {
					e.elements[optionList[index].value] = true;
				});
				return str;
			}
			...
			return this.each(function(){
				...
				var height = $this.attr('size') ? $this.attr('size')*25 : 10*25;
		...
		addItens : function(list, keyStr, valueStr) {
			...
			return this.each(function(){
				...
				$.each(list, function() {
					...
					if (e.elements[id] == undefined) {
						optionHTML += '<option value="'+id+'">'+value+'</option>';
						leftListHTML += buildListItem(id,value);
					} else if (e.elements[id]) {
						$rightList.find('#'+id).text(value);
					} else {
						$leftList.find('#'+id).text(value);
					}
				});
